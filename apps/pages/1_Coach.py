...
<style>
    ...
    /* Desktop: keep sidebar pinned (SCC toggle-arrow fix) */
    ...
    /* Mobile: sidebar collapses, columns stack vertically */
...
with st.sidebar:
    ...
    voice_choice = st.selectbox(
        ...
        index=list(TTS_VOICES.keys()).index(TTS_VOICE),
        format_func=lambda x: TTS_VOICES[x],
    )

    # Clear cached audio if voice changed
    if st.session_state.get("tts_voice_used") != voice_choice:
        st.session_state.tts_audio = None
            ...
            if not st.session_state.tts_audio:
                ...
                with st.spinner("Preparing audio..."):
                    st.session_state.tts_audio = synthesize_speech(
                        result.get("refined_answer", ""), voice=voice_choice
                    )
                    st.session_state.tts_voice_used = voice_choice
